	...
	solver(_solver)
{
	...
	{
		unary = postfix_operation | implicit_mul | function_call | function_call_string | 
			no_fences_function_call | identifier | number | implicit_string_mul | unary_operation | '(' > expression > ')';
	}
	else
	{
		unary = postfix_operation | implicit_string_mul | implicit_mul | number | function_call | function_call_string | 
